...
export default function(props: Props) {
  ...
  async function authenticate(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      let response = await axios.post('/api/person/authenticate', person);
      props.setAuthenticated(response.data.token);
      navigate('/');
    } catch (e) {
    ...
    return (
      <>
        ...
        <div className="text-center">
          Not registered yet?{' '}
          <Link className="link-primary" to={'/' + AuthMode.signup}>Sign Up</Link>
        </div>
      </>);
  } else {
    return (
      <>
        ...
        <div className="text-center">
          Already registered?{' '}
          <Link className="link-primary" to={'/' + AuthMode.signing}>Sign In</Link>
        </div>
      </>);
  }
}
