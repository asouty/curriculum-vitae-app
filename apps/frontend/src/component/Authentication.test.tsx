...
async function submitForm(emailInput: string, passwordInput: string) {
  userEvent.setup();
  let email = screen.getByPlaceholderText('Enter email');
...
describe('when the user logged on', () => {
  it('sign in', async () => {
    let token = '';
    let setAuthenticated = (receivedToken: string) => {
      token = receivedToken;
    };
    mockedAxios.post.mockResolvedValue({ data: { token: 'my_token' } });
    render(<BrowserRouter><Authentication authMode={AuthMode.signing}
                                          setAuthenticated={setAuthenticated} /></BrowserRouter>);
  ...
  it('sign up', async () => {
    ...
    render(<BrowserRouter><Authentication authMode={AuthMode.signup}
                                          setAuthenticated={setAuthenticated} /></BrowserRouter>);
